...
for line in s.split("\n"):
    if not line:
        passports.append(" ".join(next_passport))
        next_passport = []
    else:
        next_passport.append(line)

def contains(text, want):
    return text.find(want) != -1
...
def validate_passport(passport):
    ...
    for field in fields:
        ...
        if key == "ecl":
            if value not in ["amb","blu","brn","gry","grn","hzl","oth"]: return False
            else: continue
        if key == "pid":
            if not re.match(r"[0-9]{9}", value): return False
            else: continue
        if key == "eyr":
            if int(value) < 2020 or int(value) > 2030: return False
            else: continue
        if key == "hcl":
            if not re.match(r"#[0-9a-f]{6}", value): return False
            else: continue
        if key == "byr":
            if int(value) < 1920 or int(value) > 2002: return False
            else: continue
        if key == "iyr":
            if int(value) < 2010 or int(value) > 2020: return False
            else: continue
        if key == "hgt":
            if contains(value, "cm"):
                value, _ = value.split("cm")
                if int(value) < 150 or int(value) > 193: return False
                else: continue
            if contains(value, "in"):
                value, _ = value.split("in")
                if int(value) < 59 or int(value) > 76: return False
                else: continue
        if key == "cid": continue
        print("Derpy key found'", key, "'") ### WHY!?!???!?!?!?!?!? 
        return False
    return True

valid_ones = 0
for passport in passports:
    if contains(passport, "ecl:") and \
        contains(passport, "pid:") and \
        contains(passport, "eyr:") and \
        contains(passport, "hcl:") and \
        contains(passport, "byr:") and \
        contains(passport, "iyr:") and \
