s = """faded yellow bags contain 4 mirrored fuchsia bags, 4 dotted indigo bags, 3 faded orange bags, 5 plaid crimson bags.
...
def parse_string(tokens):
    parent_bag_color = tokens[0] + tokens[1]
    _ = tokens[2]
    _ = tokens[3]

    child_bag_colors = parse_child_string(tokens[4:])

    return parent_bag_color, child_bag_colors


def parse_child_string(tokens):
    child_bag_colors = []

    if tokens[0] == "no": return child_bag_colors
    
    while tokens[3][-1] == ",":
        child_bag_colors.append(tokens[1] + tokens[2])
        tokens = tokens[4:]
    child_bag_colors.append(tokens[1] + tokens[2])
    return child_bag_colors

def find_direct_parents(color_combinations, child_name):
    parents = []
    for parent, child in color_combinations:
...
color_combinations = []
for line in s.split("\n"):
    color_combinations.append(parse_string(tokenize(line)))

parents = set(find_direct_parents(color_combinations, "shinygold"))
number_of_parents = len(parents)

while True:
    to_be_added = []
    for parent in parents:
        to_be_added += find_direct_parents(color_combinations, parent)
    parents |= set(to_be_added)
